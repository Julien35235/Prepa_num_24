...
const showWeatherFromCurrentWeatherAPI=(data)=>{
    //console.log(data);
    // on cible notre span qui a l'id ville
    let ville=document.querySelector('#ville');
    // On ajoute à notre span ville, le nom de la ville qu'on trouve dans le json
    ville.innerHTML=data.name;

    // on cible la span resume
    let resume=document.querySelector('#resume')
    // on ajoute le contenu ;
    resume.innerHTML=data.weather[0].description;

    // Exercice : affichez la température dans la span id="temp"
