...
const getCurrentWeatherFromGeoLoc=(data)=>{
    //console.log(data);
    // 1) on va récupérer la propriété lat (latitude) de notre json data passé en paramètre
    ...
    let myRequest=`${protocol}${endpoint}${routeCurrentWeather}${qLat}&${qLon}&${qUnits}&${qLang}&appid=${apikey}`;
    fetch(myRequest)
        .then(
            response=>response.json()
        )
